...
my $SCRIPTDIR = '/home/NKI/b.evers/mm10/scripts';
my $GENOMEINDEX = '/home/NKI/b.evers/mm10/genome-index/mm10';
...
while (defined(my $Line=<IN>)) {
	...
	if(!$FoundInPreviousRun || ($SkipLookingAtLast && $FoundInPreviousRun > -1) || ($FoundInPreviousRun == 1 && $Depth == 1)) { 
		...
		$ExonicRelativesFound{$TargetSequence} = 0;
		`perl $SCRIPTDIR/FindRelativesOfOligo.pl -i $TargetSequence -o $PotentialRelativesFile -d $Depth -s $SeedLength`;
	}
}
...
if ($ProcessRelatives) {
	`bowtie2 $GENOMEINDEX -f $PotentialRelativesFile -t --no-hd --score-min L,-5,0 -a --mm -S $RelativesMatchFile`;	
